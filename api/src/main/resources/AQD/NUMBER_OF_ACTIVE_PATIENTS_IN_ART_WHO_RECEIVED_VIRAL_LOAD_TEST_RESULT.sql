      ...
      from
         (
            select
               ...
               GREATEST(COALESCE(data_fila, data_seguimento), COALESCE(data_seguimento, data_fila), COALESCE(data_seguimento, data_fila)) data_usar_c,
                        ...
                        from
                              ...
                              from
                                 (
                                    ...
                                    where
                                       ...
                                       and e.voided = 0 
                                       and e.encounter_datetime <= :startDate + interval 1 month - interval 1 day 
                                    ...
                                    where
                                       ...
                                       and o.value_datetime <= :startDate + interval 1 month - interval 1 day 
                                       and e.location_id = :location 
                                    group by
                                       p.patient_id 
                                 )
                                       ...
                                       from
                                          (
                                             ...
                                             from
                                                (
                                                   ...
                                                   where
                                                      ...
                                                      and p.voided = 0 
                                                      and pg.program_id = 2 
                                                      and ps.start_date <= :startDate + interval 1 month - interval 1 day 
                                                ...
                                                inner join
                                                   patient_program pg2 
                                                   on pg2.patient_id = maxEstado.patient_id 
                                                inner join
                                                   patient_state ps2 
                                                   on pg2.patient_program_id = ps2.patient_program_id 
                                             where
                                                pg2.voided = 0 
                                                and ps2.voided = 0 
                                             ...
                                             where
                                                ...
                                                and o.obs_datetime <= :startDate + interval 1 month - interval 1 day 
                                                and e.location_id = :location 
                                             group by
                                                p.patient_id 
                                          )
                                       ...
                                       from
                                          (
                                             ...
                                             where
                                                ...
                                                and death_date <= :startDate + interval 1 month - interval 1 day 
                                          )
                                          allSaida 
                                       group by
                                          patient_id 
                           ...
                           left join
                              (
                                 Select
                                    p.patient_id,
                                    max(value_datetime) data_recepcao_levantou 
                                 from
                                    patient p 
                                    inner join
                                       encounter e 
                              ...
                              max_recepcao 
                              on inicio.patient_id = max_recepcao.patient_id 
                        group by
                           inicio.patient_id 
                     )
                     inicio_fila_seg 
...
, 'N/A') valor_qualitativo 
      ...
      where
         (
(coorte12meses_final.data_estado is null 
            or 
            (
(coorte12meses_final.data_estado is not null 
               and coorte12meses_final.decisao = 2) 
               and coorte12meses_final.data_usar_c > coorte12meses_final.data_estado 
            )
) 
            and date_add(coorte12meses_final.data_usar, interval 60 day) >= :startDate + interval 1 month - interval 1 day 
         )
         OR 
         (
(coorte12meses_final.decisao = 1 
            and coorte12meses_final.data_estado < coorte12meses_final.data_fila) 
            and date_add(coorte12meses_final.data_usar, interval 60 day) >= :startDate + interval 1 month - interval 1 day 
         )
      group by
         coorte12meses_final.patient_id 
                              ...
                              from
                                 (
                                    ...
                                    where
                                       ...
                                       and e.encounter_type = 18 
                                       and e.voided = 0 
                                       and e.encounter_datetime <= :startDate + interval 2 month - interval 1 day 
                                       and e.location_id = :location 
                                    group by
                                       p.patient_id 
                                    union
                                    ...
                                    where
                                       ...
                                       and o.concept_id = 23866 
                                       and o.value_datetime is not null 
                                       and o.value_datetime <= :startDate + interval 2 month - interval 1 day 
                                       and e.location_id = :location 
                                    group by
                                       p.patient_id 
                                 )
                                       ...
                                       from
                                          (
                                             ...
                                             from
                                                (
                                                   ...
                                                   where
                                                      ...
                                                      and p.voided = 0 
                                                      and pg.program_id = 2 
                                                      and ps.start_date <= :startDate + interval 2 month - interval 1 day 
                                                      and pg.location_id = :location 
                                                   group by
                                                      p.patient_id 
                                                )
                                             ...
                                             where
                                                ...
                                                and o.obs_datetime <= :startDate + interval 2 month - interval 1 day 
                                                and e.location_id = :location 
                                             group by
                                                p.patient_id 
                                          )
                                       ...
                                       from
                                          (
                                             ...
                                             where
                                                dead = 1 
                                                and death_date is not null 
                                                and death_date <= :startDate + interval 2 month - interval 1 day 
                                          )
                                          allSaida 
                                       group by
                                          patient_id 
...
(coorte12meses_final.data_estado is not null 
            ...
            and date_add(coorte12meses_final.data_usar, interval 60 day) >= :startDate + interval 2 month - interval 1 day 
         )
         OR 
         (
(coorte12meses_final.decisao = 1 
      ...
      group by
         coorte12meses_final.patient_id       -- Aqui  Neste Bloco vamos buscar a activos do terceiro mes e aqueles que tem carga viral no terceiro mes
      union
      select
         coorte12meses_final.patient_id,
         coorte12meses_final.data_usar,
