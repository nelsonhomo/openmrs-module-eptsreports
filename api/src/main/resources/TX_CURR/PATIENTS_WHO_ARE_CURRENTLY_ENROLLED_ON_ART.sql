...
from (
            ...
            from (
						...
						from (                                                                                                                           
									...
									from (   	
												...
												) inicio_real_antigo where inicio_real_antigo.data_inicio < '2023-12-21'                                                                                                             
									) inicio                                                                                                                                    
								left join                                                                                                                                  
								( 
			...
			) inicio_fila_seg_prox  group by patient_id                                                                                                                                 
  ) coorte12meses_final                                                                                                                               
	where (data_estado is null or (data_estado is not null and  data_usar_c>data_estado)) and date_add(data_usar, interval 28 day) >=:endDate and data_inicio < '2023-12-21'
            ...
            (select inicio.*,                                                                                                                    
             ...
             from (
						...
						from (
									...
									) inicio_real  where inicio_real.data_inicio >= '2023-12-21'
						) inicio_real 
					 left join
					 (
						select inicio_real_antigo.patient_id, inicio_real_antigo.data_inicio                                                                              
						from (     
									select patient_id,min(data_inicio) data_inicio                                                                              
									from (                                                                                                                       
												 select  p.patient_id,min(e.encounter_datetime) data_inicio                                                          
					 ...
					 ) inicio_real_antigo on inicio_real_antigo.patient_id = inicio_real.patient_id where   inicio_real_antigo.patient_id is null
	          )inicio                                                                                                                                    
              left join                                                                                                                                  
             ( 
                select patient_id,max(data_estado) data_estado                                                                                              
