...
where ((data_estado is null or (data_estado is not null and  ultimo_fila > data_estado)) and date_add(maximo_proximo_fila_recepcao, interval 28 day) >=date_add(:startDate, interval -1 day)  and date_add(maximo_proximo_fila_recepcao, interval 28 day) < :endDate)
 or ((data_saida_transferencia is null or (data_saida_transferencia is not null and ultimo_fila > data_saida_transferencia))  and data_entrada_obito is null and data_entrada_transferencia >= ultimo_fila and data_entrada_transferencia <= :endDate and date_add(data_proximo_lev, interval 1 day) >=:startDate) 
 or ((data_saida_obito is null or (data_saida_obito is not null and  ultimo_fila > data_saida_obito))  and data_entrada_obito >= ultimo_fila and data_entrada_obito between :startDate and :endDate)