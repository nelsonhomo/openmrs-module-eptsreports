select patient_id, data_inicio, ultimo_fila          
from (
			select inicio_fila_seg_prox.*,         
					...
					greatest(coalesce(data_fila,data_seguimento),coalesce(data_seguimento,data_fila)) maximo_ultimo_fila_seguimento
             from (
 ...
 where ((data_estado is null or (data_estado is not null and  maximo_ultimo_fila_seguimento > data_estado)) and date_add(maximo_proximo_fila_recepcao, interval 28 day) >=date_add(:startDate, interval -1 day)  and date_add(maximo_proximo_fila_recepcao, interval 28 day) < :endDate)
  or ((data_estado is null or (data_estado is not null and  maximo_ultimo_fila_seguimento > data_estado))  and data_entrada_obito is null and data_entrada_transferencia >= maximo_ultimo_fila_seguimento and data_entrada_transferencia <= :endDate and date_add(maximo_proximo_fila_recepcao, interval 1 day) >=:startDate) 
  or ( (data_estado is null or (data_estado is not null and  maximo_ultimo_fila_seguimento > data_estado))  and data_entrada_obito >= maximo_ultimo_fila_seguimento and data_entrada_obito between :startDate and :endDate)