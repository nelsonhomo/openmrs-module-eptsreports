select entrada_por_obito.patient_id
from( 
	select entrada_por_obito.patient_id, entrada_por_obito.data_estado 
	from( 
		select entrada_por_obito.patient_id, entrada_por_obito.data_estado 
		from( 
			...
			(
				...
				where fila_seguimento.data_encountro is null or dead_state.data_estado >= fila_seguimento.data_encountro
		) entrada_por_obito
		left join
		(
		...
		) saida on entrada_por_obito.patient_id = saida.patient_id      
