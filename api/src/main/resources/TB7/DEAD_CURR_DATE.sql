	...
	from (  
		...
		from (                                                                
					...
					where pg.voided=0 and ps.voided=0 and p.voided=0 and pe.voided = 0 and pg.program_id = 2                                        
						and ps.start_date <= CURDATE() and pg.location_id =277 group by pg.patient_id                                           
		) max_estado                                                                                                                        
		...
		where e.voided=0 and o.voided=0 and p.voided=0 and pe.voided = 0                                                               
			...
			and o.obs_datetime <= CURDATE() and e.location_id=277                                                                        
		...
		(                                                                                                                              
			...
			from patient p                                                                                                              
			 ...
			 where e.voided=0 and p.voided=0 and pe.voided = 0 and e.encounter_datetime <= CURDATE()                                       
				...
				inner join obs o on o.encounter_id = e.encounter_id                                                                         
		where e.encounter_type = 21 and o.voided=0 and o.concept_id in (2031,23944,23945) and o.value_coded = 1366 and ultimaBusca.data_estado = e.encounter_datetime and e.location_id = 277 
...
left join(
	...
	from(
				...
				where   p.voided=0 and pe.voided = 0 and e.voided=0 and e.encounter_type=18                                                                      
						and e.location_id=277 and e.encounter_datetime <= CURDATE()                                                                                  
						group by p.patient_id  
				union
