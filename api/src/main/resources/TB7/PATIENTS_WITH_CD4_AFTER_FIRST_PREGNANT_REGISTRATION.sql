select cd4_eligible.patient_id from 
(
			select 
		...
		left join(
				select lactante_real.patient_id,min(lactante_real.data_parto) data_parto  
				from ( 
 ...
 left join (
	...
	where e.voided=0 and e.encounter_type in (6,13,53,51,90) and o.concept_id in (1695, 165515) and o.voided=0 
		and  e.location_id=:location and o.obs_datetime between :startDate and :endDate - interval 1 month
)cd4_absolute on cd4_eligible.patient_id = cd4_absolute.patient_id
where  cd4_absolute.obs_datetime > cd4_eligible.data_gravida and cd4_eligible.decisao =1