...
(
	...
	(
select final.patient_id, if(final.type_patient=1,'ABANDONO',if(final.type_patient=2,'MORTO',if(final.patient_id=3,'TRANSFERIDO PARA',if(final.patient_id=4,'SUSPENSO',null)))) as type_patient
 from (
select iit.patient_id, 1 type_patient from (
select patient_id         
from (
...
select dead.patient_id, 2 type_patient from (
...
)rf on rf.patient_id=txMl.patient_id
)refuse
)final
