...
select coorte_final.*
from (
select coorte_final.*, first_cd4.data_cd4
from (
...
inner join
		...
		) first_cd4 on first_cd4.patient_id = coorte_final.patient_id
	)coorte_final
	where coorte_final.data_cd4 between date_add(coorte_final.data_restart, interval - 30 day)  and date_add(coorte_final.data_restart, interval 28 day) group by coorte_final.patient_id
)coorte_final
