                          ...
                          left join 
                          (
                          select rv.patient_id,rv.data_revelacao data_revelacao,if(o.value_coded=6339,'N','P') state from 
                          (
                          select p.patient_id,max(e.encounter_datetime) data_revelacao from patient p  
                          inner join encounter e on p.patient_id=e.patient_id  
                          inner join obs o on o.encounter_id=e.encounter_id 
                          where e.voided=0 and o.voided=0 and p.voided=0 and  
                          e.encounter_type in (35) and  e.encounter_datetime<=:endDate and e.location_id=:location  and p.patient_id=418
                          group by p.patient_id
                          ) rv 
                         inner join encounter e on e.patient_id=rv.patient_id
                         inner join obs o on o.encounter_id=e.encounter_id
                         where e.encounter_type=35 and o.concept_id=6340 and o.value_coded in(6338,6339) and e.voided=0 and o.voided=0
                         group by rv.patient_id
			              ) ultimaRevelacao on coorte12meses_final.patient_id=ultimaRevelacao.patient_id
                          left join
                          (
                          ...
                          inner join obs o on o.encounter_id=e.encounter_id 
                          where e.voided=0 and o.voided=0 and p.voided=0 and  
                          e.encounter_type in (35) and o.concept_id=6340 and o.value_coded in(6337) 
                          and  e.encounter_datetime<=:endDate and e.location_id=:location 
                          )revelacaoTotal on revelacaoTotal.patient_id=coorte12meses_final.patient_id
                          
                         left join
                          (
                            select  p.patient_id,min(e.encounter_datetime) data_revelacao
                                from    patient p 
                                        inner join encounter e on p.patient_id=e.patient_id
                                        inner join obs o on e.encounter_id=o.encounter_id
                          ...
                          and date_add(coorte12meses_final.data_usar, interval 60 day) >=:endDate and floor(datediff(:endDate,p.birthdate)/365)  between 8 and 14 and revelacaoTotal.patient_id is null
