select 
  ...
  concat(
    ...
    ' ', 
    ifnull(pn.family_name, '')
  ) as NAME, 
  if(
    ultimaRevelacao.estadoRevelacao is not null, 
    ultimaRevelacao.estadoRevelacao, 
    ''
  ) AS CODE, 
  if(
    revelacaoParcial.data_revelacao is not null, 
    DATE_FORMAT(
      DATE(
        revelacaoParcial.data_revelacao
      ), 
      '%d-%m-%Y'
    ), 
    'N/A'
  ...
  if(
    revelacaoParcial.data_revelacao is not null, 
    DATEDIFF(
      :endDate, revelacaoParcial.data_revelacao
    ), 
    'N/A'
  ) as DIF_DAYS 
from 
