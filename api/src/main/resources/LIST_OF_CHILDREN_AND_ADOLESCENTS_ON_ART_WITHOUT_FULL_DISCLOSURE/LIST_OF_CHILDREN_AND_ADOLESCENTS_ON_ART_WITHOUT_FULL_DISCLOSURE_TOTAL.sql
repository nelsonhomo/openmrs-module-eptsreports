...
from 
  (
    select 
      ...
      dataNaoRevelado, 
      if(
        revelado.patient_id is not null, 
        1, 
        if(
          revelacaoParcial.patient_id is not null, 
          2, 
          if(
            naoRevelacao.patient_id is not null, 
            3, 4
          )
        )
      ) statusRevelacao 
    from 
      (
        select 
          ...
          GREATEST(
            COALESCE(data_fila, data_seguimento), 
            COALESCE(data_seguimento, data_fila), 
            COALESCE(data_seguimento, data_fila)
          ) data_usar_c, 
          GREATEST(
            COALESCE(
              data_proximo_lev, data_recepcao_levantou30
            ), 
            COALESCE(
              data_recepcao_levantou30, data_proximo_lev
            )
          ) data_usar 
        from 
          (
            select 
              ...
              date_add(
                data_recepcao_levantou, interval 30 day
                        ...
                        from 
                          (
                            ...
                            where 
                              ...
                              and e.encounter_type in (53, 6) 
                              and o.concept_id in (6272, 6273) 
                              and o.value_coded in (1706, 1366) 
                              and o.obs_datetime <= :endDate 
                              and e.location_id =:location 
                            group by 
                              p.patient_id 
                    ...
                    from 
                      (
                        ...
                        from 
                          ...
                          inner join encounter e on date(e.encounter_datetime)= date(fila.data_fila) 
                          and e.encounter_type = 18 
                          and e.voided = 0 
                          and e.patient_id = fila.patient_id
                      ) maxFila 
      ...
      left join (
        select 
          p.patient_id, 
          max(e.encounter_datetime) dataRevelacaoTotal 
        from 
          patient p 
          inner join encounter e on p.patient_id = e.patient_id 
          inner join obs o on e.encounter_id = o.encounter_id 
      ...
      left join (
        select 
          p.patient_id, 
          max(e.encounter_datetime) dataRevelacaoParcial 
        from 
          patient p 
          inner join encounter e on p.patient_id = e.patient_id 
          inner join obs o on e.encounter_id = o.encounter_id 
      ...
      left join (
        select 
          p.patient_id, 
          max(e.encounter_datetime) dataNaoRevelado 
        from 
          patient p 
          inner join encounter e on p.patient_id = e.patient_id 
          inner join obs o on e.encounter_id = o.encounter_id 
      ...
      (
        (
          (
            ...
            or (
              (
                data_estado is not null 
                and decisao = 2
              ) 
              and data_usar_c > data_estado
            )
          ) 
        ...
        OR (
          (
            decisao = 1 
            and data_estado < data_fila
          ) 
          and date_add(data_usar, interval 60 day) >= :endDate
        )
      ) 
      and floor(
        datediff(:endDate, p.birthdate)/ 365
      ) between 8 
      and 14 
      and revelado.patient_id is null
  ) revelacao