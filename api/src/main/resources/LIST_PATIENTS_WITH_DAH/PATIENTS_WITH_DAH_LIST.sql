select inicioDAH.patient_id, pid.identifier as NID, concat(ifnull(pn.given_name,''),' ',ifnull(pn.middle_name,''),' ',ifnull(pn.family_name,'')) as NomeCompleto, 
     ...
     IF(ISNULL(transferredIn.initialDate), 'Não', 'Sim') AS transferido_de,
     case 
     ...
     DATE_FORMAT(cd4EligibilidadeMDSDAH.max_data_cd4, '%d-%m-%Y') AS max_data_cd4,
     cd4EligibilidadeMDSDAH.value_numeric,
     ...
     IF(estadiamentoClinico.tipoEstadio=3, 'Estadio III', 'Estadio IV') AS tipo_estadio,
     estadiamentoClinico.motivoEstadio,
      @motivoIndice := 1 + LENGTH(estadiamentoClinico.motivoEstadio) - LENGTH(REPLACE(estadiamentoClinico.motivoEstadio, ',', '')) AS motivoIndice,
      SUBSTRING_INDEX(estadiamentoClinico.motivoEstadio, ',', 1) AS motivoEstadio1,
      IF(@motivoIndice > 1, SUBSTRING_INDEX(SUBSTRING_INDEX(estadiamentoClinico.motivoEstadio, ',', 2), ',', -1), '') AS motivoEstadio2,
      maxCD4.value_numeric lastCd4Result,
      DATE_FORMAT(maxCD4.max_data_cd4, '%d-%m-%Y') AS ultimoDataCd4,
      penultimoCd4.value_numeric penultimoResultadoCd4,
      DATE_FORMAT(penultimoCd4.penultimaDataCD4, '%d-%m-%Y') AS penultimoCd4Data,
      ultimaCV.resultadoCV,
      DATE_FORMAT(ultimaCV.data_carga, '%d-%m-%Y') AS ultimaDataCv,
      DATE_FORMAT(penultimaCV.penultimaDataCarga, '%d-%m-%Y') AS penultimaDataCv,
      penultimaCV.resultadoCV penultimoResultadoCV,
      ultimoTbLam.resultadoTbLam,
      DATE_FORMAT(ultimoTbLam.max_tblam, '%d-%m-%Y') AS ultimoTbLam,
      cragSoro.resultadoCragSoro,
      DATE_FORMAT(cragSoro.max_cragSoro, '%d-%m-%Y') AS ultimoCragSoroData,
      cragLcr.resultadoCragLcr,
      DATE_FORMAT(cragLcr.max_cragLcr, '%d-%m-%Y') AS ultimoCragLcrData,
      ultimoCacu.resultadoCacu,
      DATE_FORMAT(ultimoCacu.max_cacu, '%d-%m-%Y') AS ultimoCacuData
     from (
	...
	where transferidoPara.patient_id is null
		...
		left join (
			SELECT trasferedPatients.patient_id,initialDate FROM ( 
		...
		)trasferedPatients  GROUP BY trasferedPatients.patient_id 
...
( 
 ...
 obito on obito.patient_id = abandono.patient_id
	...
	union
	  select obito.patient_id, 2 as RespostaEstadoPermanencia from ( 
 select patient_id, max(data_obito) data_obito from ( 
 select maxEstado.patient_id,maxEstado.data_obito from ( 
 select pg.patient_id, max(ps.start_date) data_obito from patient p 
 inner join patient_program pg on p.patient_id = pg.patient_id 
...
( 
 ...
 where consultaOuARV.encounter_datetime <= obito.data_obito
 union
 		select suspenso1.patient_id, 3 as RespostaEstadoPermanencia  from ( 
		select patient_id,max(data_suspencao) data_suspencao from ( 
		select maxEstado.patient_id,maxEstado.data_suspencao from( 
		select pg.patient_id,max(ps.start_date) data_suspencao from patient p 
		inner join patient_program pg on p.patient_id=pg.patient_id 
		...
		where consultaOuARV.encounter_datetime<=suspenso1.data_suspencao
		union
				select activo.patient_id, 4 as RespostaEstadoPermanencia from (
				select patient_id, min(data_inicio) data_inicio from(
		 SELECT e.patient_id, MIN(e.encounter_datetime) AS data_inicio  FROM patient p 
		inner join encounter e on p.patient_id=e.patient_id 
		WHERE p.voided=0 and e.encounter_type=18 AND e.voided=0 and e.encounter_datetime<=:endDate and e.location_id=:location GROUP BY p.patient_id 
	...
	o.voided=0 and o.concept_id=6273 and o.value_coded=1706 and e.encounter_type=6 and  e.location_id=:location group by p.patient_id 
		...
		where transferidoPara.patient_id is null or suspenso.patient_id is null or obito.patient_id is null
	...
	left join (
	Select p.patient_id,max(o.obs_datetime) max_data_cd4, o.value_numeric  From patient p  
	inner join encounter e on p.patient_id=e.patient_id  
	inner join obs o on e.encounter_id=o.encounter_id  
	where p.voided=0 and e.voided=0 and o.voided=0 and concept_id = 1695 and  e.encounter_type in (6,51,13,53,90) 
	and o.obs_datetime between :startDate and :endDate and e.location_id=:location 
	...
	left join(
     select ultimoCd4.patient_id, max(o.obs_datetime) penultimaDataCD4, ultimoCd4.max_data_cd4, o.value_numeric from ( 
     Select p.patient_id,max(o.obs_datetime) max_data_cd4  From patient p  
	inner join encounter e on p.patient_id=e.patient_id  
	inner join obs o on e.encounter_id=o.encounter_id  
	where p.voided=0 and e.voided=0 and o.voided=0 and concept_id = 1695 and  e.encounter_type in (6,51,13,53,90) 
	and o.obs_datetime between :startDate and :endDate and e.location_id=:location 
	group by p.patient_id 
	) ultimoCd4 inner join encounter e on e.patient_id = ultimoCd4.patient_id
	inner join obs o on o.encounter_id = e.encounter_id
	where o.concept_id = 1695 and o.voided = 0 and e.voided = 0 and e.encounter_type in (6,51,13,90) 
	and date(o.obs_datetime) < ultimoCd4.max_data_cd4 group by e.patient_id
 ...
 case
                                    ...
                                    then
                                       ...
                                       null 
                                 end), obs.value_numeric) AS resultadoCV from ( 
 ...
 case
	...
	and date(o.obs_datetime) < ultimaCV.data_carga order by date(o.obs_datetime) desc
