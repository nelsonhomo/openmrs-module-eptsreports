select patient_id from ( select inicio.patient_id, inicio.data_inicio, timestampdiff(year,per.birthdate,:endDate) idade, timestampdiff(month,inicio.data_inicio,:endDate) idadeEmTarv, cd4Absoluto.value_numeric cd4Abs,cd4SemiQuantitativo.value_coded cd4SemiQt, cd4Percentual.value_numeric cd4Per, cvmenor1000.patient_id pidcv12meses,cvmenor1000.person_id pidcvmenor100  
            from (
            ...
            inner join encounter e on p.patient_id=e.patient_id  
            inner join obs o on e.encounter_id=o.encounter_id  
            where p.voided=0 and e.voided=0 and o.voided=0 and concept_id = 1695 and  e.encounter_type in (6,9,13,53,51) and o.obs_datetime between (:endDate - INTERVAL 12 MONTH) AND :endDate and e.location_id=:location group by p.patient_id 
            )max_cd4  
            ...
            inner join encounter e on p.patient_id=e.patient_id  
            inner join obs o on e.encounter_id=o.encounter_id  
            where   p.voided=0 and e.voided=0 and o.voided=0 and concept_id=730 and  e.encounter_type in (6,9,13,53,51) and o.obs_datetime between (:endDate - INTERVAL 12 MONTH) AND :endDate and e.location_id=:location  
            group by p.patient_id 
            ) max_cd4  
            left join obs o on o.person_id=max_cd4.patient_id and max_cd4.max_data_cd4=o.obs_datetime and o.voided=0 and  
            ...
            where (idade>=2 and idadeEmTarv>=6) and ((pidcvmenor100 is not null ) or (pidcv12meses is null and (idade>=5 and idade<=9) and (cd4Abs>200 or cd4SemiQt = 1254)) or (pidcv12meses is null and (idade>=2 and idade<=4) and (cd4Abs>750 or cd4Per>15)))