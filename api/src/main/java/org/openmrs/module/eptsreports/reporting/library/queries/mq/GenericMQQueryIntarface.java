    ...
    public static final String calculateAgeOnTheFirstConsultationDateBiggerThanParamOrBreastfeeding(
      ...
      String sql =
          "select patient_id from ( "
              ...
              + "select f.patient_id, "
              + "f.data_lactante, "
              + "f.data_gravida, if(f.data_lactante is null,1, if(f.data_gravida is null,2, if(f.data_gravida>=f.data_lactante,1,2))) decisao   from  (  "
              + "Select p.patient_id,gravida.data_gravida,lactante.data_lactante from ( "
              ...
              + "group by p.patient_id   "
              + ")gravida on gravida.patient_id=p.patient_id   "
              + "left join   "
