...
public interface TxCurrQueries {
  ...
  public class QUERY {
    ...
    public static final String findTxCurrPatientsOnARVDispense6OrMoreMonths =
        " select maxEnc.patient_id from (select p.patient_id,max(e.encounter_datetime) max_enc from patient p "
            + "join encounter e on p.patient_id=e.patient_id "
            + "where e.encounter_type in (6,18) and e.location_id = :location and e.encounter_datetime <= :endDate "
            + "and p.voided = 0 and e.voided = 0 "
            + "group by p.patient_id "
            + ") maxEnc "
            + "inner join ( "
            + "select patient_id, max(data_levantamento) data_levantamento,source from ( "
            + "select patient_id, data_levantamento,source from ( "
            + "select max_ficha.patient_id, max_ficha.data_levantamento, 3 as source from( "
            + " (Select p.patient_id,max(e.encounter_datetime) data_levantamento "
            + "	from 	patient p inner join encounter e on p.patient_id=e.patient_id "
            + "			inner join obs o on o.encounter_id=e.encounter_id "
            + "	where 	e.voided=0 and o.voided=0 and p.voided=0 and "
            + "			e.encounter_type =6 and o.concept_id=23739 and o.value_coded in (23888) and "
            + "			e.encounter_datetime<=:endDate and e.location_id=:location "
            ...
            + "union "
            + "select max_ficha.patient_id, max_ficha.data_mdc data_levantamento, 2 as source from "
            ...
            + "where  e.encounter_type in(6) and e.location_id=:location and o.concept_id=165174 and o.value_coded in(23888, 165314) and o.voided=0 "
            ...
            + " inner join patient patient_without_fila on patient_without_fila.patient_id = max_ficha.patient_id "
            + "union "
            + "select patient_id, last_levantamento data_levantamento, 1 as source from ( "
            ...
            + "and datediff(obs_proximo_levantamento.value_datetime,pickup.last_levantamento) > 173 "
            + "group by pickup.patient_id "
            + ") maxFila "
            + ") maxFilaOrFichaClinica order by patient_id, data_levantamento, source "
            + ") maxSource group by maxSource.patient_id "
            + ") correctEvaluated on correctEvaluated.patient_id = maxEnc.patient_id "
            + "where correctEvaluated.data_levantamento = maxEnc.max_enc ";

    public static final String findTxCurrPatientsOnARVDispense3And5MoreMonths =
        " select maxEnc.patient_id from (select p.patient_id,max(e.encounter_datetime) max_enc from patient p "
            + "join encounter e on p.patient_id=e.patient_id "
            + "where e.encounter_type in (6,18) and e.location_id = :location and e.encounter_datetime <= :endDate "
            + "and p.voided = 0 and e.voided = 0 "
            ...
            + "			inner join obs o on o.encounter_id=e.encounter_id "
            + "	where 	e.voided=0 and o.voided=0 and p.voided=0 and "
            + "			e.encounter_type =6 and o.concept_id=23739 and o.value_coded in (23720) and "
            + "			e.encounter_datetime<=:endDate and e.location_id=:location "
            + "	group by p.patient_id ) max_ficha "
            + "inner join patient patient_without_fila on patient_without_fila.patient_id = max_ficha.patient_id) "
            + "union "
            ...
            + "join obs o on o.encounter_id=e.encounter_id "
            + "join obs obsEstado on obsEstado.encounter_id=e.encounter_id "
            + "where  e.encounter_type in(6) and e.location_id=:location and o.concept_id=165174 and o.value_coded in(23720) and o.voided=0 "
            + "and grupo.concept_id=165323  and grupo.voided=0 and obsEstado.concept_id=165322  and obsEstado.value_coded in(1256,1257) "
            + "and obsEstado.voided=0  and grupo.voided=0 "
            + "and grupo.obs_id=o.obs_group_id and grupo.obs_id=obsEstado.obs_group_id  and e.encounter_datetime<=:endDate "
            + "group by p.patient_id "
            ...
            + "inner join obs obs_proximo_levantamento on pickup.encounter_id=obs_proximo_levantamento.encounter_id "
            + "where obs_proximo_levantamento.concept_id = 5096  and obs_proximo_levantamento.voided = 0 "
            + "and datediff(obs_proximo_levantamento.value_datetime,pickup.last_levantamento) between 83 and 173 "
            + "group by pickup.patient_id "
            + ") maxFila "
            + ") maxFilaOrFichaClinica order by patient_id, data_levantamento, source "
            + ") maxSource group by maxSource.patient_id "
    ...
    public static final String findTxCurrPatientsOnARVDispenseForLessThan3Months =
        " select maxEnc.patient_id from (select p.patient_id,max(e.encounter_datetime) max_enc from patient p "
            + "join encounter e on p.patient_id=e.patient_id "
            + "where e.encounter_type in (6,18) and e.location_id = :location and e.encounter_datetime <= :endDate "
            + "and p.voided = 0 and e.voided = 0 "
            ...
            + "select patient_id, max(data_levantamento) data_levantamento,source from ( "
            + "select patient_id, data_levantamento,source from ( "
            + "select max_ficha.patient_id, max_ficha.data_levantamento, 2 as source from( "
            + " (Select p.patient_id,max(e.encounter_datetime) data_levantamento "
            + "	from 	patient p inner join encounter e on p.patient_id=e.patient_id "
            + "			inner join obs o on o.encounter_id=e.encounter_id "
            + "	where 	e.voided=0 and o.voided=0 and p.voided=0 and "
            + "			e.encounter_type =6 and o.concept_id=23739 and o.value_coded in (1098) and "
            + "			e.encounter_datetime<=:endDate and e.location_id=:location "
            + "	group by p.patient_id ) max_ficha "
            + "inner join patient patient_without_fila on patient_without_fila.patient_id = max_ficha.patient_id) "
            + "union "
            ...
            + "inner join obs obs_proximo_levantamento on pickup.encounter_id=obs_proximo_levantamento.encounter_id "
            + "where obs_proximo_levantamento.concept_id = 5096  and obs_proximo_levantamento.voided = 0 "
            + "and datediff(obs_proximo_levantamento.value_datetime,pickup.last_levantamento) < 83 "
            + "group by pickup.patient_id "
            + ") maxFila "
            + ") maxFilaOrFichaClinica order by patient_id, data_levantamento, source "
            + ") maxSource group by maxSource.patient_id "
