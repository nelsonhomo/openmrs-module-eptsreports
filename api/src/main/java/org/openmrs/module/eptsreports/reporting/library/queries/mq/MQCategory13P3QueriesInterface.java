    ...
    public static final String
        findAllPatientsWhoDroppedOutARTAtTheEndOfFirstSixMonthsAfterInitiatedART =
            "SELECT art_start.patient_id  FROM ( "
                ...
                + "inner join "
                + "( "
                + "select patient_id, max(data_proxima_consulta) data_proxima_consulta from ( "
                + "Select 	ultimavisita.patient_id, o.value_datetime as data_proxima_consulta "
                + "from "
                ...
                + "union "
                + "Select 	p.patient_id,(max(value_datetime) + INTERVAL 30 day) data_proxima_consulta "
                + "from 	patient p "
                + "		inner join encounter e on p.patient_id=e.patient_id "
                + "		inner join obs o on e.encounter_id=o.encounter_id "
                + "where 	p.voided=0 and e.voided=0 and o.voided=0 and e.encounter_type=52 and "
                ...
                + ") abandonos "
                + "group by patient_id "
                + ") abandonos on abandonos.patient_id = art_start.patient_id "
                + "where date_add(data_proxima_consulta, interval 60 day) <= date_add(art_start.art_start_date, interval 6 MONTH) ";
    ...
    public static final String
        findPatientsDeclaredDeadAtTheEndOfSixMonthsAfterChangeRegimenInFirstLineART =
            " select alternativa.patient_id "
                ...
                + "			inner join obs obsLinha on obsLinha.encounter_id=e.encounter_id "
                + "	where 	p.voided=0 and e.voided=0 and e.encounter_type=53 and obsLinha.concept_id=21190 and obsLinha.voided=0 and "
                + "			obsLinha.obs_datetime BETWEEN :startInclusionDate and :endRevisionDate  and e.location_id=:location "
                + "	group by p.patient_id "
                + " ) alternativa "
                + "inner join ( "
                ...
                + " where obito.data_obito <= date_add(alternativa.data_linha23898, INTERVAL 6 MONTH) ";
    ...
    public static final String
        findPatientsWhoDroppedOutAtTheEndOfSixMonthsOfRegimenChangeInFirstLine =
            " select alternativa.patient_id "
                ...
                + ") abandonos "
                + "group by patient_id "
                + ") abandonos on abandonos.patient_id = alternativa.patient_id "
                + "where date_add(data_proxima_consulta, interval 60 day) <= date_add(alternativa.data_linha21190, interval 6 MONTH) ";
    ...
    public static final String
        findPatientsDroppedOutARTInTheFirstSixMonthsAfterInitiatedSecondLineART =
            " select alternativa.patient_id "
                ...
                + "group by patient_id "
                + ") abandonos on abandonos.patient_id = alternativa.patient_id "
                + "where date_add(data_proxima_consulta, interval 60 day) <= date_add(alternativa.data_linha21187, interval 6 MONTH) ";
  }
}
