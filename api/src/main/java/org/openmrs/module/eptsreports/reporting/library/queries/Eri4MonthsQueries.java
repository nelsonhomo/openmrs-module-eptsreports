  ...
  public static final String
    ...
    String query =
        "SELECT patient_id FROM (SELECT inicio_real.patient_id, "
            + "inicio_real.art_start_date, "
            + "max(lev_seg.encounter_datetime) last_encounter, "
            + "IF(saida.data_estado is not null and max(date(ultLevantamento.dataUltimoLevantamento))>saida.data_estado,null,saida.state_id) final_state, "
            + "IF(saida.data_estado is not null and max(date(ultLevantamento.dataUltimoLevantamento))>saida.data_estado,null,saida.data_estado) final_statedate, "
            + "IF(date_add(date(ultLevantamento.dataProxLevantamento), interval 59 day) < :endDate,'LTFU',null) ltfu "
            + "FROM( "
            ...
            + ") tx_new where art_start_date between date_add(date_add(:endDate, interval -5 month), INTERVAL 1 DAY) and date_add(:endDate, interval -4 month)  and art_start_date < '2023-12-21' "
            + "	union "
            ...
            + "INNER JOIN encounter e on p.patient_id=e.patient_id "
            + "where p.voided=0 and e.voided=0 and  e.encounter_type = 18 AND e.encounter_datetime between (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) and :endDate and e.location_id=:location "
            + "UNION "
            ...
            + ") lev_seg "
            + "ON lev_seg.patient_id = inicio_real.patient_id and lev_seg.encounter_datetime BETWEEN date_add(inicio_real.art_start_date, interval 61 day) and date_add(inicio_real.art_start_date, interval 120 day "
            + ") "
            + "LEFT JOIN "
            + "( "
            + "select patient_id, data_estado, state_id from "
            + "( "
            ...
            + "INNER JOIN patient_program pg ON p.patient_id=pg.patient_id "
            + "inner join patient_state ps on pg.patient_program_id=ps.patient_program_id "
            + "WHERE pg.voided=0 and ps.voided=0 and p.voided=0 AND pg.program_id=2 AND ps.state = 10 AND ps.end_date is null AND ps.start_date between  (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) and :endDate and location_id=:location "
            + "group by pg.patient_id "
            + "union "
            ...
            + "o.concept_id in (6272,6273) and o.value_coded = 1366 AND o.obs_datetime between  (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) and :endDate and e.location_id=:location "
            + "GROUP BY p.patient_id "
            + "union "
            ...
            + "and obitos.data_estado between (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) and :endDate "
            ...
            + "INNER JOIN patient_program pg ON p.patient_id=pg.patient_id "
            + "inner join patient_state ps on pg.patient_program_id=ps.patient_program_id "
            + "WHERE pg.voided=0 and ps.voided=0 and p.voided=0 AND pg.program_id=2 AND ps.state = 8 AND ps.end_date is null AND ps.start_date between  (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) and :endDate and location_id=:location "
            + "group by pg.patient_id "
            + "union "
            ...
            + "INNER JOIN obs  o on e.encounter_id=o.encounter_id "
            + "WHERE e.voided=0 and o.voided=0 AND p.voided=0 AND e.encounter_type in (53,6) and "
            + "o.concept_id in (6272,6273) and o.value_coded = 1709 AND o.obs_datetime between  (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) and :endDate and e.location_id=:location "
            + "GROUP BY p.patient_id "
            + ")suspensos "
            + "inner join "
            + "( "
            ...
            + "and suspensos.data_estado between (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) and :endDate "
            ...
            + "LEFT JOIN ( "
            + "select patient_id,encounter_datetime dataUltimoLevantamento,max(data_prox_levantamento) dataProxLevantamento from ( "
            + "Select ultimavisita.patient_id,ultimavisita.encounter_datetime,o.value_datetime data_prox_levantamento from ( "
            ...
            + "union "
            + "select p.patient_id, value_datetime data_ultimo_levantamento, date_add(max(value_datetime), interval 30 day) data_prox_levantamento "
            + "from patient p "
            + "inner join person pe on pe.person_id = p.patient_id "
            + "inner join encounter e on p.patient_id=e.patient_id "
            + "inner join obs o on e.encounter_id=o.encounter_id "
            + "where p.voided=0 and pe.voided = 0 and e.voided=0 and o.voided=0 and e.encounter_type=52 "
            + "and o.concept_id=23866 and o.value_datetime is not null and e.location_id=:location and o.value_datetime between (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) and :endDate "
            + "group by p.patient_id "
            + ") lev "
            + "group by patient_id "
            + ") ultLevantamento on ultLevantamento.patient_id=inicio_real.patient_id "
            + "LEFT JOIN ( "
            + "SELECT f.patient_id FROM ( "
            + "SELECT maxState.patient_id, ps.start_date FROM ( "
            + "SELECT p.patient_id, pg.patient_program_id, ps.start_date as maxStateDate FROM patient p "
            + "inner join patient_program pg on p.patient_id=pg.patient_id "
            + "inner join patient_state ps on pg.patient_program_id=ps.patient_program_id "
