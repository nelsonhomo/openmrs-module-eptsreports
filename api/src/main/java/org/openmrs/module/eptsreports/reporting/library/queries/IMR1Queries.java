...
public interface IMR1Queries {
  ...
  public class QUERY {
    ...
    public static final String findPatientsWhoAreNewlyEnrolledOnArtCareByAgeRange =
        "                    																							"
            ...
            + "            AND o.concept_id=1190 AND o.value_datetime is NOT NULL AND o.value_datetime <= :location AND e.location_id=:location GROUP BY p.patient_id            "
            + "            UNION                                                                                                                                                 "
            + "            SELECT pg.patient_id, MIN(date_enrolled) art_start_date FROM patient p                                                                                "
            + "            INNER JOIN patient_program pg ON p.patient_id=pg.patient_id                                                                                           "
            + "            WHERE pg.voided=0 AND p.voided=0 AND program_id=1 AND date_enrolled <= :endDate AND location_id=:location GROUP BY pg.patient_id                      "
    ...
    public static final String findPatientsAgeByTheDateOfEnrollmentOnARTCare =
        "select patient_id "
            + "from "
            + "	( "
            + "	select patient_id,min(art_enrolled_date) art_enrolled_date "
            ...
            + "	group by patient_id "
            + "	) art_init_real "
            + "	INNER JOIN person pe ON art_init_real.patient_id=pe.person_id "
            + "	where art_enrolled_date between (:endDate - INTERVAL 2 MONTH  + INTERVAL 1 DAY) and (:endDate - INTERVAL 1 MONTH ) "
            + "	and TIMESTAMPDIFF(year,birthdate,art_enrolled_date) BETWEEN %d AND %d   AND birthdate IS NOT NULL ";
  }
}
