  ...
  class QUERY {
    ...
    public static final String
        findPatientsWhoDroppedOutARTInFirstThreeMonthsPreviousConsultationMarkedAsPregnant =
            " SELECT gravida.patient_id FROM ( "
                + " SELECT tx_new.patient_id,firstConsult.dataConsulta "
                + " FROM ( "
                + " SELECT patient_id, MIN(art_start_date) art_start_date FROM  ( "
                + " SELECT p.patient_id, MIN(value_datetime) art_start_date FROM patient p "
                + " INNER JOIN encounter e ON p.patient_id = e.patient_id "
                ...
                + "where e.voided=0 and o.voided=0 and p.voided=0 and  e.encounter_type in (53,6) and o.concept_id in (6272,6273) and o.value_coded in (1707) and e.location_id=:location "
                + "group by p.patient_id "
                + " ) abandono on abandono.patient_id = gravida.patient_id "
                + "where abandono.data_estado BETWEEN date_sub(gravida.dataConsulta, INTERVAL 3 month) AND gravida.dataConsulta ";

    public static final String findPregnantPatientsWhoReceivedViralLoadResultInTheAvaliationPeriod =
        "SELECT firstConsult.patient_id FROM ( "
            + " SELECT tx_new.patient_id "
            + " FROM (SELECT patient_id, MIN(art_start_date) art_start_date FROM  ( "
            + " SELECT p.patient_id, MIN(value_datetime) art_start_date FROM patient p "
            + " INNER JOIN encounter e ON p.patient_id = e.patient_id "
            ...
            + " WHERE firstConsult.dataConsulta >= :startInclusionDate AND firstConsult.dataConsulta <= :endInclusionDate "
            + " )firstConsult "
            + " INNER JOIN encounter e ON e.patient_id = firstConsult.patient_id AND e.voided = 0 AND e.encounter_type = 6 AND e.location_id = :location "
            + " INNER JOIN obs o ON o.encounter_id = e.encounter_id AND o.voided = 0 "
    ...
    public static final String
        findPregnantPatientsWhoReceivedViralLoadResultInTheAvaliationPeriodAnd33DaysAfterRequestLaboratoryInvestigations =
            "SELECT resultado.patient_id FROM ( "
                + " SELECT firstConsult.patient_id,min(e.encounter_datetime) dataResultado FROM ( "
                + " SELECT tx_new.patient_id "
                + " FROM (SELECT patient_id, MIN(art_start_date) art_start_date FROM  ( "
                + " SELECT p.patient_id, MIN(value_datetime) art_start_date FROM patient p "
                + " INNER JOIN encounter e ON p.patient_id = e.patient_id "
                ...
                + " AND resultado.dataResultado > e.encounter_datetime AND datediff(resultado.dataResultado, e.encounter_datetime) <= 33 ";
  }
}
