  ...
  public class QUERY {
    ...
    public static final String findNumberOfUsersWhoseInitiatedPrepForTheFirstTime =
        "                                                                           "
            ...
            + "and pp.location_id = :location  and ps.start_date between :startDate and :endDate  and ps.end_date is null "
            + "	union 																																			"
    ...
    public static final String findNumberOfUsersWhoseRestartedPREP =
        "																							"
            ...
            + "		inner join patient_program pp on pp.patient_id = max_estado.patient_id															 			"
            + "	    	inner join patient_state ps on ps.patient_program_id = pp.patient_program_id and ps.start_date = max_estado.data_estado	        		"
            + "	where pp.program_id = 25 and ps.state = 76 and pp.voided = 0 and ps.voided = 0 and pp.location_id = :location  and ps.end_date is null		 							"
            + "	union 																																			"
            + "	select p.patient_id, e.encounter_datetime  																										"
            + "	from patient p  																																"
            + "		inner join encounter e on e.patient_id = p.patient_id 																						"
    ...
    public static final String findNumberOfUseresCurrentlyOnPrep =
        "																									"
            ...
            + "      	and	(:endDate - INTERVAL 1 MONTH)    and ps.end_date is null																																"
            + "		union 																																			 						"
            + "		select p.patient_id, e.encounter_datetime  																										 						"
            + "		from patient p  																																 						"
            + "			inner join encounter e on e.patient_id = p.patient_id 																						 						"
