  ...
  class QUERY {
    ...
    public static String findPatientsWithConsultationOrDrugPickUpWithoutFichaResumo =
        "SELECT "
            ...
            + " MIN(DATE_FORMAT(fichaClinica.encounter_datetime, '%d-%m-%Y')) AS primeiraFichaClinica, "
            + "DATE_FORMAT(fichaClinica.date_created, '%d-%m-%Y') AS dataRegistoFichaClinica, "
            + "MIN(DATE_FORMAT(fila.encounter_datetime, '%d-%m-%Y')) AS dataPrimeiroFila, "
            + "MIN(DATE_FORMAT(recepcao.dataLevantamento, '%d-%m-%Y')) AS dataPrimeiroRecepcao, "
            ...
            + "where  p.voided = 0 and e.voided = 0 and e.encounter_type = 6 and e.encounter_datetime >= '2019-11-01' and e.location_id =:location "
            + ") fichaClinica  on fichaClinica.patient_id = pe.person_id "
            + " left join( "
            + "Select p.patient_id, e.encounter_datetime, l.name  location_name, e.date_created from  patient p "
            + "inner join encounter e on p.patient_id = e.patient_id "
            + "inner join location l on l.location_id = e.location_id "
            + "where  p.voided = 0 and e.voided = 0 and e.encounter_datetime >= '2019-11-01' and e.encounter_type = 18 and e.location_id =:location "
            + ") fila  on fila.patient_id = pe.person_id "
            + "left join ( "
            + "SELECT  p.patient_id, o.value_datetime dataLevantamento, e.encounter_datetime dataRegistoFicha, l.name location_name FROM  patient p "
            + "INNER JOIN encounter e on p.patient_id = e.patient_id "
            + "inner join obs o on e.encounter_id = o.encounter_id "
            + "inner join location l on l.location_id = e.location_id "
            + "where  p.voided = 0 and e.voided = 0 and o.voided = 0 and e.encounter_type = 52 "
            + "AND o.concept_id = 23866 and o.value_datetime is not null and o.value_datetime >= '2019-11-01' and e.location_id =:location "
