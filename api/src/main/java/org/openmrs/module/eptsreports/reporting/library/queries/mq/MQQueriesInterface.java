  ...
  class QUERY {
    ...
    public static final String findPatientWhoStartTPINHDuringPeriodCategory7 =
        " SELECT DISTINCT patient_id FROM "
            + " ( "
            + " SELECT p.patient_id,MAX(obsStart.value_datetime) FROM patient p "
            + " INNER JOIN encounter e ON e.patient_id = p.patient_id "
            ...
            + " AND obsStart.concept_id=6128 AND obsStart.voided=0 "
            + " AND DATE(obsStart.value_datetime) BETWEEN :startInclusionDate AND :endInclusionDate "
            + " GROUP BY p.patient_id "
            + " UNION "
    ...
    public static final String findPatientWhoCompleteTPI3HPCategory7 =
        "select DISTINCT patient_id from ( "
            ...
            + "obsTBActiva.obs_datetime dataTBActiva, obsRastreio.obs_datetime dataRastreioPositivo, obsTB.obs_datetime dataTB from (  "
            + "SELECT patient_id, MAX(dataInicioTPI) dataInicioTPI FROM (  "
            + " SELECT p.patient_id,MAX(obsStart.value_datetime) AS dataInicioTPI FROM patient p "
            + " INNER JOIN encounter e ON e.patient_id = p.patient_id "
            + " INNER JOIN obs obsTPI ON obsTPI.encounter_id = e.encounter_id "
            + " INNER JOIN obs obsStart on obsStart.encounter_id=e.encounter_id "
            + " WHERE p.voided = 0 AND e.voided = 0 AND obsTPI.voided = 0 AND e.encounter_type = 53 AND e.location_id = :location "
