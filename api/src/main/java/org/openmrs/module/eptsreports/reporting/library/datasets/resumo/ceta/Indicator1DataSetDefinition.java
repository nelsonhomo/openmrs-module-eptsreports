...
@Component
public class Indicator1DataSetDefinition extends BaseDataSet {
  ...
  public DataSetDefinition constructDataset() {
    ...
    dataSetDefinition.setName("Indicator 1");
    ...
    CohortDefinition indicator1 = this.resumoMensalCETACohortQueries.getPatientsIndicator1();

    dataSetDefinition.addDimension(
        "indicator1Dimension",
        EptsReportUtils.map(
            this.resumoMensalCETADimensions.getResumoMensalCETADimension(indicator1), mappings));

    dataSetDefinition.setName("Resumo Mensal CETA");
    dataSetDefinition.addParameters(this.getParameters());

    final CohortIndicator rmCETAIndicator1 =
        this.eptsGeneralIndicator.getIndicator("RMCETA", EptsReportUtils.map(indicator1, mappings));

    addRow(
        dataSetDefinition,
        "I1",
        "Nr de Pacientes rastreados usando o FICA-BEM",
        EptsReportUtils.map(rmCETAIndicator1, mappings),
        resumoMensalCETADataSetDefinition.getColumns("indicator1Dimension"));
