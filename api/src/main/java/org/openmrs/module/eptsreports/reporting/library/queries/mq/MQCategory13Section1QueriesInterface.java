  ...
  class QUERY {
    ...
    public static final String findPatientsWhoAbandonedARTInTheFirstSixMonthsOfARTStart =
        "SELECT art_start.patient_id FROM ( "
            ...
            + "where abandono.data_estado between art_start.art_start_date and date_add(art_start.art_start_date, interval 6 MONTH) ";

    public static final String findPatientsWhoReinitiatedARTForAtLeastSixMonthsWithoutDroppOutART =
        " select reinicio_tarv.patient_id from ( "
            + "select p.patient_id, max(o.obs_datetime) data_reinicio_tarv from patient p "
            + "inner join encounter e on p.patient_id=e.patient_id "
            + "inner join obs  o on e.encounter_id=o.encounter_id "
            + "where e.voided=0 and o.voided=0 and p.voided=0 and  e.encounter_type = 6 and o.concept_id = 6273 and o.value_coded = 1705 and o.obs_datetime>=:startInclusionDate and o.obs_datetime<=:endRevisionDate and e.location_id=:location "
            + "group by p.patient_id "
            + " ) reinicio_tarv "
            + "inner join "
            + " ( "
            ...
            + "where TIMESTAMPDIFF(MONTH, reinicio_tarv.data_reinicio_tarv, abandono.data_estado) >= 6 ";

    public static final String
        findPatientsWhoAbandonedARTInTheFirstSixMonthsAfterChangeFirstLineRegimenART =
            " Select linhaAlternativa.patient_id,linhaAlternativa.dataLinha,abandono.data_estado "
                + " from ( "
                + "Select enc.patient_id,enc.encounter_datetime ultimaConsulta,max(obsLinha.obs_datetime) dataLinha from ( "
                + "Select p.patient_id,max(e.encounter_datetime) encounter_datetime from patient p "
                + "inner join encounter e on p.patient_id=e.patient_id "
                ...
                + "where abandono.data_estado between linhaAlternativa.dataLinha and date_add(linhaAlternativa.dataLinha, interval 6 MONTH) ";
    ...
    public static final String
        findPatientsWhoAbandonedARTInTheFirstSixMonthsAfterInitiatedSecondLineRegimenART =
            " Select segundaLinha.patient_id from ( "
                ...
                + " inner join encounter e on p.patient_id=e.patient_id "
                + " where p.voided = 0 and e.voided = 0 and e.encounter_type = 6 and "
                + " e.encounter_datetime > :startInclusionDate and e.encounter_datetime <= :endRevisionDate and e.location_id = 236 "
                + " group by p.patient_id "
                + " ) enc "
                + " inner join encounter e on e.patient_id = enc.patient_id "
                + " inner join obs obsLinha on obsLinha.encounter_id = e.encounter_id "
                + " where obsLinha.concept_id = 21187 and e.encounter_type = 53 "
                + " and obsLinha.voided = 0 and e.voided = 0 and obsLinha.obs_datetime < enc.encounter_datetime and e.location_id = 236 "
                + " group by enc.patient_id "
                + " ) segundaLinha "
                + " inner join "
                + " ( "
                ...
                + "inner join encounter e on p.patient_id=e.patient_id "
                + "inner join obs  o on e.encounter_id=o.encounter_id "
                + "where e.voided=0 and o.voided=0 and p.voided=0 and  e.encounter_type in (53,6) and o.concept_id in (6272,6273) and o.value_coded in (1707) and o.obs_datetime>=:startInclusionDate and o.obs_datetime<=:endRevisionDate and e.location_id=:location "
                + "group by p.patient_id "
                + " ) abandono on abandono.patient_id = segundaLinha.patient_id "
                + "where abandono.data_estado between segundaLinha.dataLinha and date_add(segundaLinha.dataLinha, interval 6 MONTH) ";
