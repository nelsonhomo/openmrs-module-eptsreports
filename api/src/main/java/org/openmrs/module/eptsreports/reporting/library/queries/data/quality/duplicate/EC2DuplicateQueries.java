  ...
  class QUERY {
    ...
    public static String findPatiendsWithDuplicatedNID =
        " select duplicatedDemographic.patient_id, "
            ...
            + "						 from ( "
            + "SELECT  repeated.patient_id, repeated.gender, repeated.birthdate, repeated.given_name, repeated.family_name,repeated.identifier, repeated.person_id, repeated.person_name_id,repeated.patient_identifier_id "
            + "from ( "
            + "SELECT pe.person_id, pn.person_name_id, pi.patient_identifier_id,pe.gender, pe.birthdate, pn.given_name, pn.family_name,pi.identifier,pi.identifier_type "
            + "FROM patient p "
            ...
            + "group by  pe.gender,pe.birthdate,pn.given_name, pn.family_name "
            + "having count(pe.gender) >=2 and count(pe.birthdate) >=2 and count(pn.given_name) >= 2 and count(pn.family_name) >=2 "
            + ") sameDemographic "
            + "inner join ( "
            ...
            + "inner join ( "
            + "SELECT pe.gender, pe.birthdate, pn.given_name, pn.family_name,pi.identifier,p.patient_id,pe.person_id, pn.person_name_id, pi.patient_identifier_id "
            + "FROM patient p "
            + "inner join person pe on (pe.person_id = p.patient_id and pe.voided = 0) "
            + "inner join person_name pn on (pn.person_id = p.patient_id and pn.voided = 0) "
            + "inner join patient_identifier pi on (pi.patient_id = p.patient_id and pi.voided = 0) "
            + ") repeated on repeated.gender = sameDemographic.gender and sameDemographic.birthdate = repeated.birthdate and sameDemographic.given_name = repeated.given_name and sameDemographic.family_name = repeated.family_name "
            ...
            + "					AND o.concept_id=1255 AND o.value_coded=1256 AND e.encounter_datetime<=curdate() AND e.location_id=:location GROUP BY p.patient_id "
            + "					UNION "
            ...
            + "					AND o.concept_id=1190 AND o.value_datetime is NOT NULL AND o.value_datetime<=curdate() AND e.location_id=:location GROUP BY p.patient_id "
            + "					UNION "
            + "					SELECT pg.patient_id, MIN(date_enrolled) art_start_date FROM patient p "
            + "					INNER JOIN patient_program pg ON p.patient_id=pg.patient_id "
            + "					WHERE pg.voided=0 AND p.voided=0 AND program_id=2 AND date_enrolled<=curdate() AND location_id=:location GROUP BY pg.patient_id "
            + "					UNION SELECT e.patient_id, MIN(e.encounter_datetime) AS art_start_date FROM patient p "
            + "					INNER JOIN encounter e ON p.patient_id=e.patient_id "
            + "					WHERE p.voided=0 AND e.encounter_type=18 AND e.voided=0 AND e.encounter_datetime<=curdate() AND e.location_id=:location GROUP BY p.patient_id "
            + "					UNION "
            + "					SELECT p.patient_id, MIN(value_datetime) art_start_date FROM patient p "
            + "					INNER JOIN encounter e ON p.patient_id=e.patient_id "
            + "					INNER JOIN obs o ON e.encounter_id=o.encounter_id "
            + "					WHERE p.voided=0 AND e.voided=0 AND o.voided=0 AND e.encounter_type=52 "
            + "					AND o.concept_id=23866 AND o.value_datetime is NOT NULL AND o.value_datetime<=curdate() AND e.location_id=:location GROUP BY p.patient_id) "
            + "					art_start GROUP BY patient_id ) tx_new on tx_new.patient_id = patient.patient_id "
            + "					left join "
            + "			      ( "
            ...
            + "			               inner join patient_program pg on p.patient_id=pg.patient_id "
            + "			                 inner join patient_state ps on pg.patient_program_id=ps.patient_program_id "
            + "			           where pg.voided=0 and ps.voided=0 and p.voided=0 and pg.program_id=2 and location_id=:location  and ps.start_date <= curdate() "
            ...
            + "			         where p.voided=0 and e.voided=0 and e.encounter_type=53 and obsdata.value_datetime <= curdate()  and e.location_id=:location group by p.patient_id "
            ...
            + "				where  p.voided = 0 and e.voided = 0 and e.encounter_datetime <= curdate() and e.encounter_type = 18 and e.location_id =:location "
            ...
            + "				inner join location l on l.location_id = e.location_id "
            + "				where  p.voided = 0 and e.voided = 0 and o.voided = 0 and e.encounter_type = 52 "
            + "				AND o.concept_id = 23866 and o.value_datetime is not null and o.value_datetime <= curdate() and e.location_id =:location "
            + "				group by p.patient_id "
            ...
            + "			inner join encounter e on p.patient_id = e.patient_id "
            + "			inner join location l on l.location_id = e.location_id "
            + "			where  p.voided = 0 and e.voided = 0 and e.encounter_datetime <= curdate() and e.encounter_type = 6 and e.location_id =:location "
            + "			group by p.patient_id "
            ...
            + "                              between date_sub(curdate(), interval 7 MONTH) and curdate()  and location_id=:location "
            + "                              union "
            ...
            + "                              between date_sub(curdate(), interval 7 MONTH) and curdate() and e.location_id=:location "
            + "                              union "
            + "                              Select p.patient_id,o.obs_datetime data_tratamento from  patient p "
            + "                              inner join encounter e on p.patient_id=e.patient_id "
            + "                              inner join obs o on e.encounter_id=o.encounter_id "
            ...
            + "			left join ( "
            + "			               Select  p.patient_id, max(e.encounter_datetime),o.value_numeric book_number, page.value_numeric page_number, line.value_numeric line_number from  patient p "
            + "                              inner join encounter e on p.patient_id=e.patient_id "
            + "                              inner join obs o on e.encounter_id=o.encounter_id "
            ...
            + "                              and o.concept_id=23806 and page.concept_id = 6266 and line.concept_id = 6268 and e.location_id=:location "
            + "                              group by p.patient_id "
